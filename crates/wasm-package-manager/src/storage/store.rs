...
//!
//! The schema is defined in [`wasm_package_manager_migration`]; entities
...
use wasm_package_manager_migration::Migrator;
use wasm_package_manager_migration::MigratorTrait;
use wasm_package_manager_migration::entities::{
...
fn parse_kind(s: Option<&str>) -> Option<wasm_meta_registry_types::PackageKind> {
...
impl Store {
    ...
    async fn build_package_version(
        ...
        tag: Option<String>,
    ) -> anyhow::Result<wasm_meta_registry_types::PackageVersion> {
        ...
        let custom_rows = oci_manifest_annotation::Entity::find()
            ...
            .all(&self.db)
            .await?;
        let custom: Vec<wasm_meta_registry_types::AnnotationEntry> = custom_rows
            .into_iter()
            .map(|a| wasm_meta_registry_types::AnnotationEntry {
                key: a.key,
                value: a.value,
            })
            .collect();
    ...
    async fn get_world_imports(
        ...
        Ok(rows
            .into_iter()
            .map(|r| wasm_meta_registry_types::WitInterfaceRef {
    ...
    /// Build a [`ComponentSummary`](wasm_meta_registry_types::ComponentSummary)
    ...
    ) -> anyhow::Result<wasm_meta_registry_types::ComponentSummary> {
    ...
    async fn enrich_iface_docs(&self, refs: &mut [wasm_meta_registry_types::WitInterfaceRef]) {
...
fn into_queue_task(row: fetch_queue::Model) -> wasm_meta_registry_types::QueueTask {
    ...
    wasm_meta_registry_types::QueueTask {
...
impl Store {
    ...
    pub(crate) async fn get_queue_status(
        &self,
    ) -> anyhow::Result<wasm_meta_registry_types::QueueStatus> {
    ...
    pub(crate) async fn get_package_dependencies(
        ...
        Ok(rows
            .into_iter()
            .map(|r| wasm_meta_registry_types::PackageDependencyRef {
                package: r.declared_package,
                version: r.declared_version,
            })
            .collect())
...
mod component_tests {
    ...
    use wasm_meta_registry_types::{ComponentSummary, WitInterfaceRef};
